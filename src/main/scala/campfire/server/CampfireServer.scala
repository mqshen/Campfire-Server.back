...
import campfire.database.{ ReactiveMongoPlugin }
...
import campfire.database.{ MongoQuery, User, UserAuth, UserSync, OperationSync }
      ...
      case HttpRequest(HttpMethods.POST, Uri.Path("/session"), header, entity, protocol) =>
        ...
        entity.as[FormData] match {
          case Right(formData) => {
            ...
            f onSuccess {
              case user: User =>
                //userOpt.fold(sendFailed(currentSender, "user not found")) { user =>
          ...
          try {
            ...
            packets.foreach { packet =>
              SessionManager.getSessionIdByName(packet.toUserName).map { sessionId =>
                ...
                resolver ! SendMessage(sessionId, "testendpoint", Json.toJson(messageEvent).toString())
              }
            }
          } catch {
            case e: Exception =>
              ...
              syncKeys.foreach { syncKey =>
                ...
                f onSuccess {
                  case operations: List[OperationSync] =>
                    ...
                    //                      implicit val userFormat = jsonFormat4(OperationSync)
                    //                    }
                    //                    import TheJsonProtocol._
